...
import { HeroMemoryDemo } from "./HeroMemoryDemo";
import {
  CONTACT_EMAIL,
  GITHUB_URL,
  LOCAL_CORPUS,
...
const copy = {
  zh: {
    ...
    heroNote: "一份记忆，在每个编程 Agent 里接得上。",
    title: (
      <>
        在 Claude 里记住，
        <br />
    ...
    lede: "MindBridge 把工作偏好和确认过的判断存进本地 Memory Core，再通过 MCP 交给 Codex、Claude Code 和其他编程 Agent。你可以随时查询、修改、归档、查看历史，也能把一天总结成记忆卡。",
    ...
    harness: {
      eyebrow: "AGENT HARNESS · 工具发现链路",
      title: "Agent 不会读取 Python 函数；Host 把工具说明交给模型。",
      note:
        "MindBridge 通过 MCP 暴露工具。Codex / Claude Code 负责启动本地 server、列出工具、把参数 Schema 放进模型上下文，再执行模型选择的调用。",
      steps: [
        ["01 · 注册", "@mcp.tool() 注册 11 个函数；FastMCP 从函数名、docstring 与类型注解生成工具说明和 JSON Schema。"],
        ["02 · 发现", "Codex / Claude Code 启动本地 STDIO MCP server，并通过 tools/list 读取可用工具。"],
        ["03 · 选择", "Host 把工具名、用途和参数交给模型；模型看到的是调用契约，不是 Python 源码。"],
        ["04 · 执行", "模型返回工具名与参数，Host 发起 tools/call，FastMCP 再把请求分发给对应函数。"],
        ["05 · 继续", "函数调用 MemoryService 读写记忆；tool_result 回到模型上下文，Agent 据此继续回答。"],
      ] as [string, string][],
      verdictLabel: "当前判断",
      verdict: "MindBridge 是 Agent Harness 的记忆与工具层，还不是完整的通用 Harness。",
      boundary:
        "MindBridge 已负责工具契约、记忆上下文、来源追踪、参数校验与写入边界；模型循环、上下文预算、通用权限确认和失败重试仍由 Codex / Claude Code 负责。",
      levelsTitle: "怎么判断自己做到哪一层？",
      levels: [
        ["只把函数暴露给模型", "你做的是 Tool / MCP Server。"],
        ["管理上下文 → 工具发现 → 执行 → 结果 → 下一步", "你开始拥有 Agent Harness。"],
        ["再管理权限、重试、追踪与长任务生命周期", "才接近可用于生产的完整 Harness。"],
      ] as [string, string][],
    },
    reflectionSkill: {
      status: "本机已验证 · Codex Skill",
      title: "MCP 给事实，Skill 规定怎么反思。",
      note:
        "MCP 提供可引用的 T2、T3 与 Pattern Candidate 证据；Skill 是客户端侧的反思规则，选择最窄的审阅范围并规定如何组合现有 MindBridge MCP，不增加第 12 个 tool。MCP 不可用时，也不靠聊天记忆编造证据。",
      rules: [
        ["证据先于推断", "结论引用 ids 与日期；推断写明反证与不确定性。"],
        ["候选先于人格结论", "T2 与 operational / reflective T3 分开审阅；Pattern Candidate 不是人格事实。"],
        ["修改前明确确认", "archive / edit / upsert / confirm / reject 先显示准确变更，再等当前会话的明确确认。"],
      ] as [string, string][],
      boundary: "本机 Codex 配置已验证；尚未包含在公共安装器中。",
    },
  ...
  en: {
    ...
    heroNote: "One memory that follows you across coding agents.",
    title: (
      <>
        Remember it in Claude.
        <br />
        Continue in <em>Codex</em>.
      </>
    ),
    lede: "MindBridge stores working preferences and confirmed conclusions in a local Memory Core, then makes them available to Codex, Claude Code, and other coding agents over MCP. Query, edit, archive, inspect history, or turn a day into a memory card whenever you need.",
    ...
    harness: {
      eyebrow: "AGENT HARNESS · TOOL DISCOVERY",
      title: "The agent never reads your Python functions. The host gives it tool contracts.",
      note:
        "MindBridge exposes tools over MCP. Codex or Claude Code starts the local server, discovers those tools, places their schemas in model context, and executes the call the model selects.",
      steps: [
        ["01 · Register", "@mcp.tool() registers 11 functions. FastMCP turns each name, docstring, and typed signature into a tool definition and JSON Schema."],
        ["02 · Discover", "Codex or Claude Code starts the local STDIO MCP server and requests its available tools with tools/list."],
        ["03 · Select", "The host gives the model each tool name, purpose, and input schema. The model sees the contract—not the Python source."],
        ["04 · Execute", "The model returns a tool name and arguments. The host sends tools/call, and FastMCP dispatches the registered function."],
        ["05 · Continue", "The function uses MemoryService to read or write memory. Its tool_result returns to model context so the agent can continue."],
      ] as [string, string][],
      verdictLabel: "CURRENT VERDICT",
      verdict: "MindBridge is the memory and tool layer of an agent harness—not yet a complete general-purpose harness.",
      boundary:
        "MindBridge owns tool contracts, memory context, provenance, validation, and write boundaries. Codex or Claude Code still owns the model loop, context budget, generic approvals, and retries.",
      levelsTitle: "How do you know what layer you built?",
      levels: [
        ["You only expose functions", "You built a tool or MCP server."],
        ["You own context → discovery → execution → result → next step", "You are building an agent harness."],
        ["You also own approvals, retries, traces, and long-running task state", "You are approaching a production harness."],
      ] as [string, string][],
    },
    reflectionSkill: {
      status: "LOCAL VERIFIED · CODEX SKILL",
      title: "MCP supplies the evidence. The Skill governs reflection.",
      note:
        "MCP supplies citable T2, T3, and Pattern Candidate evidence. The Skill is a client-side review policy: it selects the narrowest review, composes the existing MindBridge MCP, and adds no 12th tool. If MCP is unavailable, it does not invent evidence from chat memory.",
      rules: [
        ["Evidence before inference", "Cite ids and dates; state counter-evidence and uncertainty for every inference."],
        ["Candidates before trait claims", "Keep T2 separate from operational and reflective T3; a Pattern Candidate is not a personality fact."],
        ["Explicit confirmation before mutation", "Show the exact change before archive, edit, upsert, confirm, or reject, then wait for explicit confirmation in the current session."],
      ] as [string, string][],
      boundary: "Codex configuration verified locally; not yet included in the public installer.",
    },
...
export function Landing({ locale }: { locale: Locale }) {
  ...
  return (
    <main className="site-page" lang={t.htmlLang}>
      ...
      <section className="harness-explainer shell" id="harness" aria-labelledby="harness-title">
        <div className="harness-head">
          <div>
            <p className="eyebrow">{t.harness.eyebrow}</p>
            <h2 id="harness-title">{t.harness.title}</h2>
          </div>
          <p>{t.harness.note}</p>
        </div>

        <div className="harness-chain" aria-label={t.harness.eyebrow}>
          {t.harness.steps.map(([title, note], index) => (
            <article key={title}>
              <span className="harness-step-index">{String(index + 1).padStart(2, "0")}</span>
              <h3>{title}</h3>
              <p>{note}</p>
              {index < t.harness.steps.length - 1 ? <CaretRight weight="bold" /> : null}
            </article>
          ))}
        </div>

        <div className="harness-boundary">
          <div className="harness-verdict">
            <span>{t.harness.verdictLabel}</span>
            <strong>{t.harness.verdict}</strong>
            <p>{t.harness.boundary}</p>
          </div>
          <div className="harness-levels">
            <h3>{t.harness.levelsTitle}</h3>
            {t.harness.levels.map(([condition, result]) => (
              <div key={condition}>
                <Check weight="bold" />
                <p><strong>{condition}</strong><span>{result}</span></p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="reflection-skill shell" aria-labelledby="reflection-skill-title">
        <div className="reflection-skill-head">
          <div>
            <p className="reflection-skill-status"><ShieldCheck weight="fill" />{t.reflectionSkill.status}</p>
            <h2 id="reflection-skill-title">{t.reflectionSkill.title}</h2>
          </div>
          <p>{t.reflectionSkill.note}</p>
        </div>
        <div className="reflection-skill-rules">
          {t.reflectionSkill.rules.map(([title, note], index) => {
            const Icon = [ShieldCheck, Sparkle, PlugsConnected][index];
            return (
              <article key={title}>
                <span className="reflection-skill-icon"><Icon weight="fill" /></span>
                <div>
                  <strong>{title}</strong>
                  <p>{note}</p>
                </div>
        ...
        <p className="reflection-skill-boundary"><Sparkle weight="fill" />{t.reflectionSkill.boundary}</p>
      </section>
